...
public class Show {
    ...
    public Show clone() {
		...
		public static void quickSort(List<Show> lista, int inicio, int fim, int[] comparacoes, int[] movimentacoes) {
		    if (inicio < fim) {
		        int p = particionar(lista, inicio, fim, comparacoes, movimentacoes);
		        quickSort(lista, inicio, p - 1, comparacoes, movimentacoes);
		        quickSort(lista, p + 1, fim, comparacoes, movimentacoes);
		    }
		}
		
		private static int particionar(List<Show> lista, int inicio, int fim, int[] comparacoes, int[] movimentacoes) {
		    Date pivo = lista.get(fim).getDATE_ADDED();
		    int i = inicio - 1;
		
		    for (int j = inicio; j < fim; j++) {
		        comparacoes[0]++;
		        Date atual = lista.get(j).getDATE_ADDED();
		
		        if (compararDatas(atual, pivo) <= 0) {
		            i++;
		...
		private static int compararDatas(Date d1, Date d2) {
    ...
    public static void main(String[] args) {
	    ...
	    quickSort(listaShows, 0, listaShows.size() - 1, comparacoes, movimentacoes);
	    long fim = System.currentTimeMillis();
	
	    for (int i = 0; i < Math.min(10, listaShows.size()); i++) {
		    listaShows.get(i).imprimir();
		}
