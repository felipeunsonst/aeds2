...
public class Show {
	...
	public static void selectionSort(List<Show> lista, int[] comparacoes, int[] movimentacoes) {
    int n = lista.size();
    ...
    for (int i = 0; i < n - 1; i++) {
        ...
        for (int j = i + 1; j < n; j++) {
            ...
            String tituloMenor = lista.get(menorIndice).getTITLE().replaceAll("\"", "").toLowerCase();

            if (tituloAtual.compareTo(tituloMenor) < 0) {
                menorIndice = j;
            }
        }
    ...
    public static void main(String[] args) {
	    ...
	    selectionSort(listaShows, comparacoes, movimentacoes);
