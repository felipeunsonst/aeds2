...
public class Show {
	...
	public static void insertionSort(List<Show> lista, int[] comparacoes, int[] movimentacoes) {
		int n = lista.size();
		for (int i = 1; i < n; i++) {
			...
			while (j >= 0) {
				comparacoes[0]++;
				String tipoAtual = lista.get(j).getTYPE().replaceAll("\"", "").toLowerCase();
				String tipoChave = chave.getTYPE().replaceAll("\"", "").toLowerCase();

				int tipoCmp = tipoAtual.compareTo(tipoChave);
    ...
    public static void main(String[] args) {
	    ...
	    insertionSort(listaShows, comparacoes, movimentacoes);
